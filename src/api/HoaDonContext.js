...
export const HoaDonProvider = ({ children }) => {
    ...
    const fetchHoaDons = useCallback(
        async (pageNumber = 1, pageSize = 10, status = 'ChoGiaoHang') => {
            ...
            try {
                const response = await axios.get(`https://localhost:7111/api/Hdb/${endpoint}`, {
    ...
    return <HoaDonContext.Provider value={{ invoices, loading, fetchHoaDons }}>{children}</HoaDonContext.Provider>;
};

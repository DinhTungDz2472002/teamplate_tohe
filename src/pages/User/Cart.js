...
function Cart() {
    ...
    useEffect(() => {
        const selectedCartItems =
            cart?.chiTietGioHang?.filter((item) => selectedItems.includes(item.maChiTietGH)) || [];
    ...
    const getImageSrc = (anhSp) => (anhSp ? `/assets/images/${anhSp}` : '/assets/images/placeholder.jpg');
                                ...
                                <div className="flex justify-end">
                                    <Link
                                        to={{
                                            pathname: '/checkout',
                                            state: {
                                                selectedItems: cart.chiTietGioHang.filter((item) =>
                                                    selectedItems.includes(item.maChiTietGH),
                                                ),
                                                totalPrice: totalPrice + 10000, // Bao gồm phí vận chuyển
                                            },
                                        }}

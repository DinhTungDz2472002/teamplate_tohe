...
const OrderChoXacNhan = () => {
    ...
    const handlePayment = async (maHdb, tongTienHdb) => {
        try {
            const token = localStorage.getItem('token');
            if (!token) {
            ...
            const response = await axios.post(
                'https://localhost:7111/api/VnPay/create-payment',
                { amount: tongTienHdb, maHdb: maHdb },
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
            ...
            if (response.data.code === '00') {
                window.location.href = response.data.data; // Redirect to VnPay payment URL
            } else {
                toast.error(response.data.message || 'Không thể tạo URL thanh toán.');
            }
        } catch (error) {
            console.error('Error creating payment:', error);
            const errorMessage = error.response?.data?.message || 'Có lỗi xảy ra khi tạo thanh toán.';
            toast.error(errorMessage);
        }
    };
                            ...
                            <div className="flex space-x-4 mb-6">
                                {hoaDon.pttt === 'Chờ thanh toán' && (
                                    <button
                                        ...
                                        onClick={() => handlePayment(hoaDon.maHdb, hoaDon.tongTienHdb)}
                                    >
                                        Thanh toán
                                    </button>
                                )}

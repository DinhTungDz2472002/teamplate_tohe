// được nhưng chưa có mua ngay trong giỏ hàng
...
function ListProducts() {
    const { addToCart, fetchCart, cart } = useContext(CartContext);
        ...
        try {
            ...
            if (search || maLoai) {
                const response = await axios.post(
                    ...
                    {},
                    {
                        params: { s: search || undefined, maLoai: maLoai || undefined },
                    },
                );
                filteredProducts = response.data;
                totalItems = filteredProducts.length;
    ...
    const handleBuyNow = async () => {
        ...
        try {
            const response = await addToCart(selectedProduct.maSanPham, quantity);
            ...
            if (response && response.chiTietGioHang) {
                ...
                await fetchCart();
                newCartItem = cart?.chiTietGioHang?.find((item) => item.maSanPham === selectedProduct.maSanPham);

                if (!newCartItem) {
                    console.error('Không tìm thấy sản phẩm trong giỏ hàng:', selectedProduct.maSanPham);
                    newCartItem = {
                        maChiTietGH: Date.now(),
                        maSanPham: selectedProduct.maSanPham,
                        sanPham: selectedProduct.tenSanPham,
                        anhSp: selectedProduct.anhSp,
                        giaSanPham: selectedProduct.giaSanPham,
                        slSP: quantity,
                        donGia: selectedProduct.giaSanPham,
                    };
                }
            }
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 mt-4">
                                                ...
                                                <button
                                                    ...
                                                    className="block rounded-sm bg-gray-900 px-6 py-3 text-sm font-medium text-white transition hover:scale-105"
                                                >
                                                    Mua ngay
                                                </button>
                                            </div>
